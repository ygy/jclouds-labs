...
public class VirtualMachineApiMockTest extends BaseAzureComputeApiMockTest {
   ...
   // FIXME:
   // - Where will "vmId" be - will it be in the "properties" or at the top-level?
   // - Is additionalUnattendContent ever a single instance, rather than a list (as per
   //   the response example in https://docs.microsoft.com/en-us/rest/api/compute/virtualmachines/virtualmachines-create-or-update)
   // - ManagedDiskParameters.Id: is it really a capital I?!
   // - I tried copying the example responses from the microsoft docs, but they are not valid json
   //   (e.g. missing comma and wrong quotes in things like
   //   https://docs.microsoft.com/en-us/rest/api/compute/virtualmachines/virtualmachines-create-or-update).
   //   Therefore I backed off that.
   ...
   // for where the example json response was copied from
   ...
   private VirtualMachineProperties getProperties() {
      ...
      OSProfile.WindowsConfiguration.WinRM winrm = OSProfile.WindowsConfiguration.WinRM.create(
            ImmutableList.of(
                  OSProfile.WindowsConfiguration.WinRM.ProtocolListener.create("https", "url-to-certificate")));
